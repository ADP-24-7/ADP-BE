create table runtime.digital_asset_mismatch_case (
    ...
    constraint chk_digital_asset_mismatch_digests
        check (expected_projection_digest ~ '^[0-9a-f]{64}$' and actual_projection_digest ~ '^[0-9a-f]{64}$'),
    constraint chk_digital_asset_mismatch_fields
        check (jsonb_typeof(mismatched_fields) = 'array'
            ...
            and mismatched_fields <@ '["CUSTOMER_TOKEN", "ACCOUNT_TOKEN", "WALLET_ADDRESS", "ASSET_ID",
                "AMOUNT", "KYC_STATUS", "AML_STATUS", "WALLET_VERIFIED", "EXTERNAL_REQUEST_ID",
                "UNEXPECTED_FIELD"]'::jsonb)
);
