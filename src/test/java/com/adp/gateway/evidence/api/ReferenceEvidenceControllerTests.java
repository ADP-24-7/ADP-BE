...
class ReferenceEvidenceControllerTests {
    ...
    void ingestsReplaysAndReadsScopedReferenceEvidenceWithoutChangingRuntimePolicy() throws Exception {
        ...
        Integer policyRows = jdbcClient.sql("""
                select count(*) from policy.lifecycle_artifact where artifact_id = :evidenceId
                """).param("evidenceId", evidenceId).query(Integer.class).single();
        assertThat(policyRows).isZero();
    }

    @Test
    void rejectsDigestTamperingAndReferenceOnlyPolicyBinding() throws Exception {
        String suffix = UUID.randomUUID().toString().replace("-", "").substring(0, 12).toUpperCase();
        ...
        mockMvc.perform(post("/api/admin/reference-evidence/bundles")
            ...
            .andExpect(status().isUnprocessableEntity())
            .andExpect(jsonPath("$.reasonCode").value("REFERENCE_EVIDENCE_BINDING_INVALID"));
    }
    ...
    private ObjectNode bundle(String evidenceId, String workload) {
        ...
        evidence.put("source_ref", "analysis.ipynb");
        ...
        evidence.put("source_locator", "cell-1");
        ...
        evidence.putArray("policy_artifact_refs");
        evidence.put("content_digest", evidenceDigest(evidence));
