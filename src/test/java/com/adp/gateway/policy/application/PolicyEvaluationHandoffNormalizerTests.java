...
class PolicyEvaluationHandoffNormalizerTests {
    ...
    private final PolicyEvaluationHandoffNormalizer normalizer = new PolicyEvaluationHandoffNormalizer();

    @Test
    void preservesDaHandoffDispositionSeparatelyFromBeRuntimePolicyAction() {
        NormalizedPolicyEvaluation normalized = normalizer.normalize(artifact(), PolicyAction.REVIEW);

        assertThat(normalized.handoffDisposition()).isEqualTo("candidate_handoff");
        assertThat(normalized.evaluation().policyAction()).isEqualTo(PolicyAction.REVIEW);
