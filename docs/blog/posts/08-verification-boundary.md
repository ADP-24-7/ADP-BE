---
title: "8편. 539개 테스트보다 중요한 것은 어디까지 검증했다고 말할 수 있는가였다"
...
프로젝트의 테스트 메서드는 현재 기준으로 약 539개다. Java main source, migration, Python 분석 코드, React feature까지 합치면 숫자는 더 커진다. 하지만 파일과 테스트 개수만으로 시스템의 신뢰도를 설명할 수는 없다.
...
하지만 각 저장소의 최신 `main`을 임의로 조합하면 contract가 달라질 수 있다. 이를 막기 위해 repository lock에 각 commit SHA와 Flyway current version을 고정한다.
