...
- `measurementType`: `HTTP_FULL_RESPONSE`, `HTTP_ATTEMPT_NO_RESPONSE`, `NOT_ATTEMPTED`, `MOCK` 구분
...
- `providerHttpStatus`: 응답을 받은 경우의 bounded HTTP status code
...
- `evidenceStatus`: Connector 측정 후 `PARTIAL`, initial latency까지 저장되면 `COMPLETE`
...
- Initial latency 저장 실패는 Runtime terminal 결과를 변경하지 않고 별도 metric으로 관측한다.
