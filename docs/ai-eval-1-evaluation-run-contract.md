...
- Case Input: 서버 소유 canonical input digest로 고정
...
- 동일 Case ID에 다른 canonical input을 사용하면 Retrieval 전에 차단한다.
...
- Run Version, Dataset, Policy, Case Input, Model/Destination binding 전체의 contract digest를 생성한다.
- Idempotency request hash에 contract digest와 actual input digest를 포함해 변경된 평가 계약의 과거 결과를 replay하지 않는다.
