...
합산하면 의미가 왜곡된다. Butterfly는 원본 필드와 `KEEP` 원문 유지 필드를 비교하고, 최소화율은 `KEEP` 이외의
변환·마스킹 전략 적용 비율로 계산한다. 응답 탐지 건수는 구성비에 합치지 않고 별도로 보여준다.
분류되지 않은 응답 탐지는 `UNKNOWN`으로 유지하고 임의의 데이터 클래스에 배정하지 않는다.
